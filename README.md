...
Recent implementation note: `pnpm playbook test-triage --input <path|-> [--json|--markdown]` now acts as the first-class failure summarization surface for CI/test output, normalizing Vitest failures, pnpm recursive workspace failures, and GitHub annotation lines into copy-paste-ready `.playbook/failure-summary.json` plus `.playbook/failure-summary.md` artifacts while leaving raw logs untouched for auditability.

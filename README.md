...
Recent implementation note: `pnpm playbook docs consolidate-plan --json` now compiles `.playbook/docs-consolidation.json` into the first-class `.playbook/docs-consolidation-plan.json` reviewed-write artifact, emitting apply-compatible managed-write tasks only for conflict-free deterministic seams while routing missing anchors, mixed seams, and other ambiguous cases into explicit exclusions. Protected singleton docs still mutate only through `pnpm playbook apply --from-plan .playbook/docs-consolidation-plan.json`, so consolidation planning does not become a shadow executor.
Recent implementation note: high-signal text surfaces such as `docs consolidate`, `remediation-status`, `improve`, and `status` now default to compact operator briefs that answer decision/status, affected surfaces, blockers, and next action, while keeping full machine detail in JSON output and `.playbook/*` artifacts so human review stays fast without inventing workflow-local truth.

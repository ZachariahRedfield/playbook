...
Recent implementation note: `pnpm playbook test-fix-plan --from-triage <artifact> --json` now exposes the bounded remediation seam after diagnosis, turning only pre-approved low-risk `test-triage` findings into stable apply-compatible tasks while preserving risky or unsupported findings as explicit exclusions.

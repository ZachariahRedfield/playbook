...
export type ExecutionObservedStatus =
  | "succeeded"
  | "failed"
  | "partial"
  | "not_run";
export type ExecutionComparisonStatus =
  | "success"
  | "failed"
  | "partial_success"
  | "mismatch"
  | "not_run";
export type ArtifactDeltaChangeType =
  | "created"
  | "updated"
  | "unchanged"
  | "missing"
...
export type ExecutionVerificationSummary = {
  ...
  repos_needing_retry: string[];
  planned_vs_actual_drift: Array<{
    prompt_id: string;
    repo_id: string;
    expected: ReadinessLifecycleStage;
    observed: ReadinessLifecycleStage;
  }>;
};
...
const stablePlanDigest = (plan: FleetCodexExecutionPlan): string =>
  createHash("sha256")
    .update(JSON.stringify({ ...plan, generated_at: "__stable__" }))
    .digest("hex");
...
const ARTIFACT_PATHS: Record<
  ExecutionArtifactEvidence["artifact_kind"],
  string
> = {
...
const artifactEvidenceForStage = (
  from: ReadinessLifecycleStage,
  to: ReadinessLifecycleStage,
  ...
  if (
    ...
    deltas.push({
      artifact_kind: "repo-index",
      artifact_path: ARTIFACT_PATHS["repo-index"],
      change_type: from === to ? "unchanged" : "created",
      evidence:
        "Observed lifecycle stage requires a valid repo-index artifact.",
    });
  }
  if (to === "planned_apply_pending" || to === "ready") {
    deltas.push({
      artifact_kind: "plan",
      artifact_path: ARTIFACT_PATHS.plan,
      change_type: from === to ? "unchanged" : "created",
      evidence: "Observed lifecycle stage requires a valid plan artifact.",
    });
  }
  if (to === "ready") {
    deltas.push({
      artifact_kind: "policy-apply-result",
      artifact_path: ARTIFACT_PATHS["policy-apply-result"],
      change_type: from === to ? "unchanged" : "created",
      evidence:
        "Observed lifecycle stage requires a valid policy-apply-result artifact.",
    });
  }
  return deltas;
};
...
const observedTransitionForPrompt = (
  ...
  const item = queue.work_items.find(
    (entry) =>
      entry.repo_id === prompt.repo_id &&
      entry.parallel_group.replace(/\s+/g, "_") ===
        prompt.prompt_id.split(":")[1],
  );
...
const comparePromptOutcome = (
  ...
  if (reachedTarget && declaredStatus === "succeeded" && verificationPassed)
    return "success";
...
const summarizeWaveStatus = (
  results: ExecutionPromptResult[],
): ExecutionWaveResult["status"] => {
  if (
    results.length === 0 ||
  ...
  if (
    results.every(
      (result) => result.status === "failed" || result.status === "not_run",
    )
  )
    return "failed";
  return "partial";
...
export const buildFleetExecutionReceipt = (
  ...
  options?: {
    generatedAt?: string;
    workflowPromotion?: WorkflowPromotion | null;
  },
): FleetExecutionReceipt => {
  const generatedAt = options?.generatedAt ?? new Date().toISOString();
  const inputByPrompt = new Map(
    outcomeInput.prompt_outcomes.map((entry) => [entry.prompt_id, entry]),
  );
  ...
  const promptResults: ExecutionPromptResult[] = plan.codex_prompts.map(
    (prompt) => {
      ...
      const evidence = [
        ...
        ...(input?.artifact_deltas?.map(
          (artifact) => `${artifact.artifact_kind}:${artifact.change_type}`,
        ) ??
          artifactEvidenceForStage(observed.from, observed.to).map(
            (artifact) => `${artifact.artifact_kind}:${artifact.change_type}`,
          )),
      ];
      return {
        ...
        notes:
          input?.notes ??
          (status === "not_run"
            ? "No execution outcome was ingested for this prompt."
            : "Execution outcome ingested."),
        evidence,
      };
    },
  );
  ...
  const waveResults: ExecutionWaveResult[] = plan.waves.map((wave) => {
    const results = promptResults.filter(
      (result) => result.wave_id === wave.wave_id,
    );
    const drift = results
      .filter((result) => result.status === "mismatch")
      .map(
        (result) =>
          `${result.prompt_id}:${result.intended_transition.to}->${result.observed_transition.to}`,
      );
    return {
      wave_id: wave.wave_id,
      status: summarizeWaveStatus(results),
      completed_prompts: results
        .filter((result) => result.status === "success")
        .map((result) => result.prompt_id),
      failed_prompts: results
        .filter(
          (result) =>
            result.status === "failed" || result.status === "mismatch",
        )
        .map((result) => result.prompt_id),
      partial_prompts: results
        .filter((result) => result.status === "partial_success")
        .map((result) => result.prompt_id),
      repos_needing_retry: sortStrings(
        results
          .filter((result) =>
            ["failed", "mismatch", "partial_success"].includes(result.status),
          )
          .map((result) => result.repo_id),
      ),
      planned_vs_actual_drift: drift,
    };
  });

  const repoResults: ExecutionRepoResult[] = fleet.repos_by_priority.map(
    (repo) => {
      const results = promptResults.filter(
        (result) => result.repo_id === repo.repo_id,
      );
      const top =
        results
          .sort((left, right) => left.prompt_id.localeCompare(right.prompt_id))
          .at(-1) ?? null;
      const status: ExecutionComparisonStatus = results.some(
      ...
      return {
        ...
        observed_transition: top?.observed_transition ?? {
          from: repo.lifecycle_stage,
          to: repo.lifecycle_stage,
        },
        status,
        prompts: sortStrings(results.map((result) => result.prompt_id)),
        blockers: sortStrings(
          blockers
            .filter((blocker) => blocker.repo_id === repo.repo_id)
            .map((blocker) => blocker.blocker_code),
        ),
        retry_recommended: ["failed", "mismatch", "partial_success"].includes(
          status,
        ),
      };
    },
  ...
  const drift = promptResults
    ...
    .map((result) => ({
      ...
      expected: result.intended_transition.to,
      observed: result.observed_transition.to,
    }));
  ...
  return {
    ...
    verification_summary: {
      ...
      verification_passed_count: promptResults.filter(
        (result) => result.verification_passed,
      ).length,
      succeeded_count: promptResults.filter(
        (result) => result.status === "success",
      ).length,
      failed_count: promptResults.filter((result) => result.status === "failed")
        .length,
      partial_count: promptResults.filter(
        (result) => result.status === "partial_success",
      ).length,
      mismatch_count: promptResults.filter(
        (result) => result.status === "mismatch",
      ).length,
      not_run_count: promptResults.filter(
        (result) => result.status === "not_run",
      ).length,
      repos_needing_retry: sortStrings(
        repoResults
          .filter((result) => result.retry_recommended)
          .map((result) => result.repo_id),
      ),
      planned_vs_actual_drift: drift,
    },

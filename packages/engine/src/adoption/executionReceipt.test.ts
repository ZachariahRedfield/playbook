...
const makeFleet = (
  overrides?: Partial<FleetAdoptionReadinessSummary>,
...
const makeOutcomeInput = (
  prompt_outcomes: FleetExecutionOutcomeInput["prompt_outcomes"],
...
describe("buildFleetExecutionReceipt", () => {
  it("records a successful prompt outcome when observed lifecycle matches planned transition", () => {
    const queue = buildFleetAdoptionWorkQueue(makeFleet(), {
      generatedAt: "2026-01-01T00:00:00.000Z",
    });
    const plan = buildFleetCodexExecutionPlan(queue, {
      generatedAt: "2026-01-02T00:00:00.000Z",
    });
    const prompt = plan.codex_prompts.find(
      ...
      makeOutcomeInput([
        {
          ...
          notes: "apply completed",
        },
      ]),
      { generatedAt: "2026-01-04T00:00:00.000Z" },
    );
    const result = receipt.prompt_results.find(
      (entry) => entry.prompt_id === prompt!.prompt_id,
    );
      ...
      makeOutcomeInput([
        {
          ...
          blockers: [
            {
              blocker_code: "verify_failed",
              message: "verify remained red",
              evidence: ".playbook/verify.json",
            },
          ],
        },
      ]),
  ...
  it("records partial wave completion when only some prompts finish", () => {
    ...
    const receipt = buildFleetExecutionReceipt(
      ...
      makeOutcomeInput([
        {
          prompt_id: plan.codex_prompts[0]!.prompt_id,
          repo_id: plan.codex_prompts[0]!.repo_id,
          lane_id: plan.codex_prompts[0]!.lane_id,
          status: "partial",
          verification_passed: false,
  ...
  it("prefers explicit observed transitions from ingested execution outcomes over fleet state inference", () => {
    const queue = buildFleetAdoptionWorkQueue(makeFleet(), {
      generatedAt: "2026-01-01T00:00:00.000Z",
    });
    const plan = buildFleetCodexExecutionPlan(queue, {
    ...
    ).toEqual({ from: "planned_apply_pending", to: "planned_apply_pending" });
    expect(
      receipt.prompt_results.find(
        (entry) => entry.prompt_id === prompt!.prompt_id,
      )?.status,

...
const commandRunners: Record<string, (context: CommandContext) => Promise<CommandRunResult>> = {
  ...
  status: async ({ cwd, ci, format, quiet, commandArgs }) => {
    ...
    const scope = scopeArg === 'fleet' ? 'fleet' : scopeArg === 'queue' ? 'queue' : scopeArg === 'execute' ? 'execute' : scopeArg === 'receipt' ? 'receipt' : scopeArg === 'updated' ? 'updated' : scopeArg === 'bootstrap' ? 'bootstrap' : 'repo';

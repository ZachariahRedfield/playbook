  ...
  safeRecordRepositoryEvent(() => {
    for (const lane of [...assignments.lanes].sort((left, right) => left.lane_id.localeCompare(right.lane_id))) {
      recordWorkerAssignment(cwd, {
        ...
        related_artifacts: [WORKER_ASSIGNMENTS_PATH, ...(lane.assigned_prompt ? [lane.assigned_prompt] : [])]
      });

      recordExecutionOutcome(cwd, {
        lane_id: lane.lane_id,
        ...
        related_artifacts: [WORKER_ASSIGNMENTS_PATH]
      });
    }
  });

...
const LIFECYCLE_CANDIDATES_PATH =
  `${MEMORY_ROOT}/lifecycle-candidates.json` as const;
...
const resolveRepoName = (projectRoot: string): string => {
  const packageJson = safeReadJson<{ name?: unknown }>(
...
const staleCutoffMs = (staleDays: number): number =>
...
const readEvidenceRecords = (
  ...
  listJsonFiles(path.join(projectRoot, MEMORY_EVENTS_DIR)).flatMap(
    (filePath) => {
      ...
      const fingerprint =
        typeof parsed.eventFingerprint === "string"
...
const readCandidateRecords = (
  ...
  return parsed.candidates.flatMap((candidate) => {
    ...
    const fingerprints = provenanceEntries
      ...
      .filter((value): value is string => value !== null);
    const runId = provenanceEntries.find(
      (item) => typeof item.runId === "string",
      ...
      withLifecycle(
        {
          ...
          status: isStaleCandidate(entry.lastSeenAt, staleDays)
            ? "stale"
            : "active",
          provenance: {
            repo,
          ...
          metadata: {
            ...
            lastSeenAt:
              typeof entry.lastSeenAt === "string"
                ? toIsoDate(entry.lastSeenAt)
                : null,
            supersession: entry.supersession ?? null,
          },
        },
        {
          state: isStaleCandidate(entry.lastSeenAt, staleDays)
            ? "stale"
            : "candidate",
          warnings: isStaleCandidate(entry.lastSeenAt, staleDays)
            ? ["Candidate is stale and should be revalidated before reuse."]
            : [],
        },
      ),
    ];
...
const readPromotedRecords = (
  projectRoot: string,
  repo: string,
): KnowledgeRecord[] =>
  KNOWLEDGE_PATHS.flatMap((relativePath) => {
    const parsed = safeReadJson<MemoryKnowledgeArtifact>(
        ...
        withLifecycle(
          {
            ...
            type:
              status === "superseded"
                ? ("superseded" as KnowledgeArtifactType)
                : ("promoted" as KnowledgeArtifactType),
            createdAt: toIsoDate(
              entry.promotedAt,
              toIsoDate(parsed.generatedAt),
            ),
            repo,
            source: {
            ...
            provenance: {
              ...
              sourceCommand: null,
              runId:
                (provenanceEntries.find(
                  (item) => typeof item.runId === "string",
                )?.runId as string | undefined) ?? null,
              sourcePath: sourcePaths[0] ?? relativePath,
              eventIds,
              evidenceIds: [...eventIds],
              fingerprints: [
                ...new Set([
                  ...
                  ...toStringArray(entry.sourceEventFingerprints),
                ]),
              ].sort((left, right) => left.localeCompare(right)),
              relatedRecordIds: [...new Set(relatedRecordIds)].sort(
            ...
            metadata: {
              ...
              sourceEventFingerprints: toStringArray(
...
const readLifecycleCandidateRecords = (
  projectRoot: string,
  repo: string,
): KnowledgeRecord[] => {
  ...
  return parsed.candidates.flatMap((value) => {
    ...
    if (
      typeof entry.recommendation_id !== "string" ||
      typeof entry.target_pattern_id !== "string"
    )
      return [];
    const evidence = Array.isArray(entry.source_evidence)
      ? (entry.source_evidence as LifecycleEvidenceRecord[])
      : [];
    const evidenceIds = evidence
      .map((item) =>
        typeof item.evidence_id === "string" ? item.evidence_id : null,
      )
      .filter((item): item is string => item !== null);
    const sourcePaths = evidence
      .map((item) =>
    ...
    return [
      withLifecycle(
          ...
          provenance: {
            ...
            fingerprints: [
              ...new Set([
                ...evidence
                  .map((item) =>
                    typeof item.payload_fingerprint === "string"
            ...
            relatedRecordIds: [
              entry.target_pattern_id,
              ...toStringArray(entry.source_evidence_ids),
            ].sort((a, b) => a.localeCompare(b)),
          },
          metadata: {
            ...
            explainability: Array.isArray(entry.explainability)
              ? entry.explainability
              : [],
            sourceEvidence: Array.isArray(entry.source_evidence)
              ? entry.source_evidence
              : [],
            derivedFrom: Array.isArray(entry.derived_from)
              ? entry.derived_from
              : [],
        ...
        },
        {
          state: "candidate",
          warnings: [
            "Lifecycle recommendations are candidate-only until explicit human promotion, demotion, or retirement.",
          ],
          supersedes: [],
          supersededBy: [],
        },
...
const readGlobalPatternRecords = (projectRoot: string): KnowledgeRecord[] => {
  ...
  const parsed = safeReadJson<GlobalPatternArtifact>(store.resolvedPath);
  ...
  return parsed.patterns.flatMap((value) => {
    ...
    const provenanceRecord =
      ...
      !Array.isArray(entry.provenance)
        ...
        : {};
    const supersededBy =
      typeof entry.superseded_by === "string" && entry.superseded_by
        ? [entry.superseded_by]
        : [];
    const lifecycleEvents = Array.isArray(entry.lifecycle_events)
      ? (entry.lifecycle_events as GlobalPatternLifecycleEvent[])
      : [];
    const supersedes = lifecycleEvents
      .filter((event) => event.operation === "supersede")
      .map((event) =>
        typeof event.reason === "string" &&
        event.reason.startsWith("supersedes:")
          ? event.reason.slice("supersedes:".length).trim()
          : null,
      )
      .filter((event): event is string => Boolean(event));
    const warnings = [
      lifecycleState !== "active"
        ? `Global reusable pattern is ${lifecycleState}.`
        : "",
      lifecycleState === "demoted"
        ? "Demoted patterns remain inspectable but should not be treated as active doctrine."
        : "",
      ...
      withLifecycle(
        {
          ...
          createdAt: toIsoDate(
            typeof provenanceRecord.promoted_at === "string"
              ? provenanceRecord.promoted_at
              : (entry.retired_at ?? entry.demoted_at ?? entry.recalled_at),
            EPOCH_ISO,
          ),
          ...
          provenance: {
            ...
            relatedRecordIds: [
              ...new Set([
                ...(typeof provenanceRecord.candidate_id === "string"
                  ? [provenanceRecord.candidate_id]
                  : []),
                ...supersedes,
                ...supersededBy,
              ]),
          ...
          metadata: {
            ...
            promotedAt:
              typeof provenanceRecord.promoted_at === "string"
                ? toIsoDate(provenanceRecord.promoted_at)
                : null,
            retiredAt:
              typeof entry.retired_at === "string"
                ? toIsoDate(entry.retired_at)
                : null,
            retirementReason: entry.retirement_reason ?? null,
            demotedAt:
              typeof entry.demoted_at === "string"
                ? toIsoDate(entry.demoted_at)
                : null,
            demotionReason: entry.demotion_reason ?? null,
            recalledAt:
              typeof entry.recalled_at === "string"
                ? toIsoDate(entry.recalled_at)
                : null,
...
const hasModuleMatch = (
  ...
  if (
    typeof record.metadata.module === "string" &&
    record.metadata.module === moduleName
  ) {
    return true;
  }

  return (
    Array.isArray(record.metadata.subjectModules) &&
...
const hasRuleMatch = (record: KnowledgeRecord, ruleId: string): boolean => {
  if (
    typeof record.metadata.ruleId === "string" &&
    record.metadata.ruleId === ruleId
  ) {
    return true;
  }

  return (
    Array.isArray(record.metadata.ruleIds) &&
...
const applyKnowledgeFilters = (
  ...
  const filtered = records
    ...
    .filter((record) =>
      options.module ? hasModuleMatch(record, options.module) : true,
    )
    .filter((record) =>
      options.ruleId ? hasRuleMatch(record, options.ruleId) : true,
    )
    .filter((record) =>
      options.text ? matchesText(record, options.text) : true,
    )
    .filter((record) =>
      options.lifecycle ? record.lifecycle.state === options.lifecycle : true,
    );
...
export const buildKnowledgeSummary = (
  ...
  byLifecycle: {
    observed: records.filter((record) => record.lifecycle.state === "observed")
      .length,
    candidate: records.filter(
      (record) => record.lifecycle.state === "candidate",
    ).length,
    active: records.filter((record) => record.lifecycle.state === "active")
      .length,
    stale: records.filter((record) => record.lifecycle.state === "stale")
      .length,
    retired: records.filter((record) => record.lifecycle.state === "retired")
      .length,
    superseded: records.filter(
      (record) => record.lifecycle.state === "superseded",
    ).length,
    demoted: records.filter((record) => record.lifecycle.state === "demoted")
      .length,
  },
});
...
export const queryKnowledge = (
  projectRoot: string,
  options: KnowledgeQueryOptions = {},
): KnowledgeRecord[] => listKnowledge(projectRoot, options);
...
export const getKnowledgeProvenance = (
  ...
  return {
    record,
    evidence: sortRecords(
      records.filter(
        (entry) => entry.type === "evidence" && evidenceIds.has(entry.id),
      ),
      "desc",
    ),
    relatedRecords: sortRecords(
      records.filter(
        (entry) => entry.id !== id && relatedRecordIds.has(entry.id),
      ),
      "desc",
    ),
  };
...
export const compareKnowledge = (
  ...
  const intersect = (a: string[], b: string[]) =>
    a
      .filter((entry) => new Set(b).has(entry))
  ...
  return {
    ...
    common: {
      evidenceIds: intersect(
        left.provenance.evidenceIds,
        right.provenance.evidenceIds,
      ),
      fingerprints: intersect(
        left.provenance.fingerprints,
        right.provenance.fingerprints,
      ),
      relatedRecordIds: intersect(
        left.provenance.relatedRecordIds,
        right.provenance.relatedRecordIds,
      ),
    },
  };
};
...
export const getKnowledgeSupersession = (
  ...
  return {
    ...
    supersedes: record.lifecycle.supersedes
      ...
      .filter((entry): entry is KnowledgeRecord => Boolean(entry)),
    supersededBy: record.lifecycle.supersededBy
      .map((entry) => byId.get(entry))
      .filter((entry): entry is KnowledgeRecord => Boolean(entry)),
  };
};

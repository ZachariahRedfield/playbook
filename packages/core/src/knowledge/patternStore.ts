...
const patternKnowledgeStoreSpecs: Record<
  PatternKnowledgeScope,
  PatternKnowledgeStoreSpec
> = {
  repo_local_memory: {
  ...
  global_reusable_pattern_memory: {
    ...
    description:
      "Global promoted reusable pattern memory shared through PLAYBOOK_HOME.",
  },
  cross_repo_proposal_bridge: {
    ...
    compatibilityRelativePaths: [],
    description:
      "Cross-repo proposal bridge artifact for explicit governed promotion.",
  },
};
...
export const resolvePatternKnowledgeStore = (
  ...
  const compatibilityAbsolutePaths = spec.compatibilityRelativePaths.map(
    (relativePath) => path.join(rootPath, relativePath),
  );
  const existingCompatibilityPath = compatibilityAbsolutePaths.find(
    (candidate) => fs.existsSync(candidate),
  );
  const resolvedPath = fs.existsSync(absolutePath)
    ? absolutePath
    : (existingCompatibilityPath ?? absolutePath);
  return {
    ...spec,
    rootPath,
    absolutePath,
    compatibilityAbsolutePaths,
    resolvedFrom: fs.existsSync(absolutePath)
      ? "canonical"
      : existingCompatibilityPath
        ? "compatibility"
        : "default",
    resolvedPath,
    pathMetadata: {

...
Recent implementation note: deterministic repair planning now has its own artifact seam through `pnpm playbook test-fix-plan --from-triage <artifact> --json`, which promotes only `autofix_plan_only` findings into a stable fix-plan artifact, rejects risky findings with explicit blocked evidence, and writes the command contract needed before later orchestration wrappers can compose over test remediation safely.

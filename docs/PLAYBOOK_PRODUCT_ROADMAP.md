...
Recent implementation note: `test-triage` now also serves as the first-class failure summarization surface for CI loops, emitting deterministic `.playbook/failure-summary.json` and `.playbook/failure-summary.md` artifacts plus step-summary markdown from the same normalized contract used for diagnosis. It now parses Vitest output, pnpm recursive workspace failures, and GitHub annotation lines into stable failure classes, cross-cutting diagnoses, and recommended next checks so remediation context is copy-paste-ready without losing raw log auditability.

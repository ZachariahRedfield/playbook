...
Recent implementation note: `pnpm playbook test-fix-plan --from-triage <artifact> --json` now turns first-class `test-triage` diagnosis artifacts into stable bounded remediation artifacts, keeping low-risk auto-fix planning explicit and rejecting risky findings as review-only exclusions instead of hidden mutation behavior. `apply --from-plan` now consumes that artifact through the existing reviewed-plan execution boundary rather than creating a parallel mutation executor.

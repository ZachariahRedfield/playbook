...
- Telemetry learning note: `pnpm playbook telemetry learning-state --json` now includes additive `learning_clusters` read-model fields (`cluster_count` and cluster rows with `cluster_type`, repeated signal summary, confidence, and next review action). Text mode remains compact with brief cluster highlights, and no new execution/mutation path is introduced.

...
- `pnpm playbook docs consolidate --json` is the proposal-only integration seam for protected singleton narrative docs: workers emit fragments, the consolidator emits one compact brief plus `.playbook/docs-consolidation.json`, and no doc mutation happens automatically in v1. `pnpm playbook docs consolidate-plan --json` may prepare reviewed writes, but `apply` remains the only mutation boundary.
